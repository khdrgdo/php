...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_teacher'])) {
    ...
    if (!empty($teacher_name)) {
        // استعلام SQL لاسترجاع الجدول بناءً على اسم المدرس
        $query = "SELECT * FROM schedule WHERE teacher = ?";
        ...
        echo "<h2 class='mt-4'>Schedule Preview for Teacher: <strong>$teacher_name</strong></h2>";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            echo "<div class='alert alert-warning'>No records found for the given teacher.</div>";
        }
        echo "<a href='search_teacher.php' class='btn btn-primary'>Back</a>";
        echo "</div>";
    } else {
        echo "<div class='alert alert-danger'>Please enter a valid teacher name.</div>";
    }
}
?>
